...
date = np.load('data/train_demo_train_300000.npy')
print(date.shape)
...
y = y.ravel()
print(y.shape)
X_train, X_test, y_train, y_test = train_test_split(x, y, test_size=0.25, random_state=32)  # 随机阈值是4
print('------')
...
def caculate_main_p(pre_labels, target):
    ...
    pre_labels_reg = pre_labels_reg.astype('float64')
    target_labels_reg = np.array(target)
    target_labels_reg = target_labels_reg.astype('float64')
    # print(pre_labels_reg)
    # print(target_labels_reg)

    mse = mean_squared_error(target_labels_reg, pre_labels_reg)
    mae = mean_absolute_error(target_labels_reg, pre_labels_reg)
    r2 = r2_score(target_labels_reg, pre_labels_reg)
